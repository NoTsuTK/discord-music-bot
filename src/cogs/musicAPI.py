...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        async def test3(self, interaction):
    ...
    async def track_hook(self, event):
        if isinstance(event, lavalink.events.QueueEndEvent):
            ...
            NowPlay = nextcord.Embed(colour=0xE8F33F, title="กำลังเล่น :", description=f"<a:loading:947184673711357983> [`{event.player.current.title}`]({player.current.uri})")
            ...
            NowPlay.add_field(name=f"🔁 วนลูป :", value=f"`{loopType}`", inline=True)
            ...
            class PlayerButton(nextcord.ui.View):
                ...
                async def pause_callback(self, button, interaction):
                    ...
                    edit_msg = await chanel.fetch_message(player.fetch('Sent_message'))
                    edit_embed = edit_msg.embeds[0]

                    if(player.paused): # หยุดเล่น
                        button.label="หยุด"
                        button.emoji="⏸️"

                        edit_embed.title="กำลังเล่น :"

                        await player.set_pause(False)
                        await edit_msg.edit(embed=edit_embed, view=self)

                    else: # เล่นอยู่
                        button.label="เล่น"
                        button.emoji="▶️"

                        edit_embed.title="หยุดเล่น ⏸️ :"
                ...
                async def loop_callback(self, button, interaction):
                    ...
                    if(player.fetch('loopType') is None): # Loop one
                        ...
                        edit_embed.set_field_at(3, name="🔂 วนลูป :", value='`เพลงเดียว`')
                        ...
                        edit_embed.set_field_at(3, name="🔁 วนลูป :", value='`ทั้งหมด`')
                        ...
                        edit_embed.set_field_at(3, name="#️⃣ วนลูป :", value='`ปิด`')
                ...
                async def reduceVol_callback(self, button, interaction):
                    ...
                    if player.volume!=0:
                        edit_embed.set_field_at(4, name="🔊 ระดับเสียง :", value=f"`{player.volume}%`")

                    elif player.volume == 0 :
                        edit_embed.set_field_at(4, name="🔇 ระดับเสียง :", value=f"`ปิด`")
                ...
                async def increaseVol_callback(self, button, interaction):
                    ...
                    await edit_msg.edit(embed=edit_embed)
            ...
            if player.fetch('Sent_message') is None:
                Sent_message = await chanel.send(embed=NowPlay, view=ButtonMenu)
                player.store('Sent_message', Sent_message.id)
            else:
                edit_msg = await chanel.fetch_message(player.fetch('Sent_message'))
                await edit_msg.edit(embed=NowPlay)
    ...
    async def play(self, ctx, *, query: str):
        ...
        if results['loadType'] == 'PLAYLIST_LOADED': #playlist
    ...
    #{ctx.author.name}
            ...
            if not player.is_playing:
                NumQueue = len(player.queue)
            else:
                NumQueue = len(player.queue) + 1 
            AddQueue = nextcord.Embed(colour=0xE8F33F, title=f"✅ เพิ่มเพลงในคิว - {NumQueue}" , description=f"[`{track.title}`]({track.uri})")
            AddQueue.add_field(name="<:Request:947390209111248906> เปิดโดย", value=f"`{ctx.author.name}#{ctx.author.discriminator}`", inline=True)
            AddQueue.add_field(name="<a:Duration:947184635677392906> ความยาว", value=f"`{lavalink.format_time(track.duration)}`", inline=True)
            AddQueue.add_field(name="<:Author:947390182557106206> เจ้าของ", value=f"`{track.author}`", inline=True)
            AddQueue.set_thumbnail(url=f"https://i3.ytimg.com/vi/{track.uri[-11:]}/maxresdefault.jpg")

            await ctx.send(embed=AddQueue, delete_after=5)
        # the current track.
        if not player.is_playing:
            await player.play()
